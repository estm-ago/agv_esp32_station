...
static FnState uart_read_t(const char* logName)
{
    ...
    if (
        ...
        FNS_ERROR_CHECK(uart_trcv_buf_push(&uart_rv_pkt_buf, &uart_rv_buf));
        return FNS_OK;
    }
    else
    {
...
void uart_setup(void) {
    ...
    FNS_ERROR_CHECK_VOID(uart_trcv_buf_setup(&uart_tr_pkt_buf, UART_TRCV_BUF_CAP, UART_VEC_MAX));
    FNS_ERROR_CHECK_VOID(uart_trcv_buf_setup(&uart_rv_pkt_buf, UART_TRCV_BUF_CAP, UART_VEC_MAX));
