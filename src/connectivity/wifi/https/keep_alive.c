...
static uint64_t get_max_delay(wss_keep_alive_t alive_handle)
{
    int64_t check_after_ms = 30000; // max delay, no need to check anyone
    for (int i=0; i<alive_handle->max_clients; ++i)
    {
        if (alive_handle->clients[i].type == CLIENT_ACTIVE)
        {
            uint64_t check_this_client_at = alive_handle->clients[i].last_seen + alive_handle->keep_alive_period_ms;
...
static bool update_client(wss_keep_alive_t alive_handle, int sockfd, uint64_t timestamp)
{
    for (int i=0; i<alive_handle->max_clients; ++i)
    {
        if (alive_handle->clients[i].type == CLIENT_ACTIVE && alive_handle->clients[i].fd == sockfd)
        {
            alive_handle->clients[i].last_seen = timestamp;
            return true;
        }
    }
    return false;
...
static bool remove_client(wss_keep_alive_t alive_handle, int sockfd)
{
    for (int i=0; i<alive_handle->max_clients; ++i)
    {
        if (alive_handle->clients[i].type == CLIENT_ACTIVE && alive_handle->clients[i].fd == sockfd)
        {
            alive_handle->clients[i].type = NO_CLIENT;
            alive_handle->clients[i].fd = -1;
            return true;
        }
    }
    return false;
...
static bool add_new_client(wss_keep_alive_t alive_handle,int sockfd)
{
    for (int i=0; i<alive_handle->max_clients; ++i)
    {
        if (alive_handle->clients[i].type == NO_CLIENT)
        {
            alive_handle->clients[i].type = CLIENT_ACTIVE;
            alive_handle->clients[i].fd = sockfd;
            alive_handle->clients[i].last_seen = _tick_get_ms();
            return true; // success
        }
    }
    return false;
...
static void keep_alive_task(void* arg)
{
    ...
    while (run_task) {
        if (
            xQueueReceive(keep_alive_storage->q, (void *) &client_action,
...
wss_keep_alive_t wss_keep_alive_start(wss_keep_alive_config_t *config)
{
    ...
    if (
        xTaskCreate(keep_alive_task, "keep_alive_task", config->task_stack_size,
...
void wss_keep_alive_stop(wss_keep_alive_t alive_handle)
{
    ...
    xQueueSendToBack(alive_handle->q, &stop, 0);
    // internal structs will be de-allocated in the task
}

esp_err_t wss_keep_alive_add_client(wss_keep_alive_t alive_handle, int fd)
{
    client_fd_action_t client_fd_action = { .fd = fd, .type = CLIENT_FD_ADD};
    if (xQueueSendToBack(alive_handle->q, &client_fd_action, 0) == pdTRUE)
    {
        return ESP_OK;
    }
    return ESP_FAIL;
...
esp_err_t wss_keep_alive_remove_client(wss_keep_alive_t alive_handle, int fd)
{
...
esp_err_t wss_keep_alive_client_is_active(wss_keep_alive_t alive_handle, int fd)
{
